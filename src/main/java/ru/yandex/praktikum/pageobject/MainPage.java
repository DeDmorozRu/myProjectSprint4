...
public class MainPage {
    ...
    //Отображение элемента куки в браузере
    ...
    public void openQuestionInFaq(int index){
        WebElement questionElement = new WebDriverWait(driver, 10).until(ExpectedConditions.visibilityOfElementLocated(question));
        ...
        new WebDriverWait(driver,10).until(ExpectedConditions.elementToBeClickable(questionElementWithIndex));
        questionElementWithIndex.click();
    }

    public void clickOrderButtonAbove(){
        WebElement orderButtonElement = new WebDriverWait(driver, 10).until(ExpectedConditions.visibilityOfElementLocated(orderButtonAbove));
        orderButtonElement.click();
        new WebDriverWait(driver, 10).until(ExpectedConditions.visibilityOfElementLocated(orderModalCheck));
    }
    public void clickOrderButtonBottom(){
        WebElement orderButtonElement = new WebDriverWait(driver, 10).until(ExpectedConditions.visibilityOfElementLocated(orderButtonBottom));
        ((JavascriptExecutor)driver).executeScript("arguments[0].scrollIntoView();", orderButtonElement);
        WebElement orderButtonElementToClick = new WebDriverWait(driver, 10).until(ExpectedConditions.elementToBeClickable(orderButtonBottom));
